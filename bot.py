...
async def buy_premium_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if plan != 'yearly':
            post_kb.append([InlineKeyboardButton(
                f"⬆️ Upgrade to Yearly ({yp} VC)",
                callback_data="buy_premium_yearly")])
        post_kb.append([InlineKeyboardButton(f"🔄 Extend Current Plan", callback_data=f"buy_premium_{plan}")])
        post_kb.append([back_btn()])
        success_text += "<b>Want more?</b> Upgrade or extend below:"
